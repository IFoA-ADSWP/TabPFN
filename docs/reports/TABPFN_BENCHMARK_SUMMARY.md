...
**Verdict: TabPFN is the best risk-ranking model in the suite — AUC #1 over GLMs, LightGBM, CatBoost, XGBoost and RF on all six canonical classification datasets (deltas +0.006 to +0.033 over the best GLM, three ≥2.5 SE), with calibration (Brier) never significantly worse and log loss better or tied. It holds #1 at production scale (up to 184K rows), not just on small data. Data efficiency is a secondary strength, not its identity.**
...
| Large regression/frequency datasets (50K+ rows) | Off the parsimony frontier 5 of 12 — all at scale | Prefer GBDT/GLM |
...
> **Adopt default TabPFN (v3)** when data is scarce — ≤ ~5K training rows (8/9 size-sweep cells won, §13.2) — **or** on classification/lapse-style tasks where the linear floor is far from achievable: best LR/GLM ≥ ~3% behind the best model, or ≥ ~0.05 AUC gap. Every TabPFN win satisfies this — coil2000, bemtl16, uslapseagent, ausautoBI8999, vehvalue, and Spanish lapse (LR 0.684 vs TabPFN 0.7553, 5/5 folds, §14.10, §14.11). The classification case is not small-data-limited: TabPFN posts the best AUC of all 9 methods on all 6 canonical classification datasets, including at production scale (163K–184K rows) — AUC #1 over GLMs, LightGBM, CatBoost, XGBoost and RF (deltas +0.006 to +0.033 over the best GLM, three ≥2.5 SE), with calibration (Brier) never significantly worse and log loss better or tied (§14.11). Where the question is which policies are risky — underwriting triage, lapse/surrender propensity, claim/no-claim targeting — TabPFN wins the business case.
>
